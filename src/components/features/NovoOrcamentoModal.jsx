      ...
      <div className="bg-white rounded-[3rem] w-full max-w-4xl h-full max-h-[90vh] flex flex-col overflow-hidden shadow-2xl border border-white/20 animate-fadeIn text-slate-800">
        ...
        <div className="p-8 bg-white flex items-center justify-between border-b border-slate-50">
          <div className="flex items-center gap-6">
            ...
            <div>
              <h3 className="text-2xl font-black tracking-tight text-slate-800 uppercase">Novo Orçamento</h3>
        ...
        <div className="flex-1 flex overflow-hidden">
          {/* Sidebar de Passos */}
          <div className="w-64 bg-slate-50/50 border-r border-slate-100 p-8 hidden md:block">
                ...
                {showQuickAddClient ? (
                    <div className="bg-slate-50 p-6 rounded-[2rem] border border-slate-100 space-y-4 animate-scaleUp">
                        ...
                        <button 
                            ...
                            className="w-full py-4 bg-slate-900 text-white rounded-xl text-[10px] font-black uppercase tracking-[0.2em] shadow-lg"
                        >
                            Salvar e Continuar
                        </button>
                    </div>
                ...
                {showQuickAddVehicle ? (
                        ...
                        <div className="space-y-2">
                            ...
                            <div className="flex gap-4">
                                <button 
                                    ...
                                    className={`flex-1 p-4 rounded-2xl border-2 flex items-center justify-center gap-3 transition-all ${newVehicleData.tipo === 'CARRO' ? 'border-blue-500 bg-blue-50 text-blue-600' : 'border-slate-100 text-slate-400 bg-white'}`}
                                >
                                    <Car size={20} /> <span className="text-xs font-black uppercase">Carro</span>
                                </button>
                                <button 
                                    onClick={() => setNewVehicleData({...newVehicleData, tipo: 'MOTO'})}
                                    className={`flex-1 p-4 rounded-2xl border-2 flex items-center justify-center gap-3 transition-all ${newVehicleData.tipo === 'MOTO' ? 'border-orange-500 bg-orange-50 text-orange-600' : 'border-slate-100 text-slate-400 bg-white'}`}
                                >
                                    <Zap size={20} /> <span className="text-xs font-black uppercase">Moto</span>
                                </button>
                            </div>
                        </div>
                        <button 
                            onClick={async () => {
                                if (!newVehicleData.marca || !newVehicleData.placa) {
                                  alert('Marca e Placa são obrigatórios');
                                  return;
                                }
                                // Padroniza o tipo para caixa alta antes de salvar
                                const finalData = { ...newVehicleData, tipo: newVehicleData.tipo.toUpperCase() };
                                const result = await saveVehicle(finalData);
                                setIsSaving(false);
                                if (result.success) {
                    ...
                    <div className="grid grid-cols-1 gap-4">
                        {vehicles.length > 0 ? vehicles.map(vehicle => (
                            ...
                            <div className="p-12 text-center text-slate-400 italic bg-slate-50 rounded-[2rem] border border-dashed border-slate-200 flex flex-col items-center gap-4">
                            <p>Este cliente não possui veículos cadastrados.</p>
                            <button 
                                onClick={() => setShowQuickAddVehicle(true)}
                                className="flex items-center gap-2 px-6 py-3 bg-white border border-slate-200 rounded-xl text-xs font-black uppercase text-slate-600 hover:shadow-md transition-all"
                            >
                                <Plus size={16} /> Adicionar Veículo Agora
                            </button>
                            </div>
                        )}
                        <button onClick={() => setStep(1)} className="text-xs font-bold text-primary uppercase tracking-widest py-4 hover:underline">← Voltar para seleção de cliente</button>
                    </div>
                )}
              </div>
            )}
            ...
            {step === 3 && (
              <div className="space-y-6 animate-slideInRight">
                        ...
                        <div>
                          ...
                          <div className="flex items-center gap-2 mt-0.5">
                            ...
                            <span className={`text-[8px] font-black uppercase px-1.5 py-0.5 rounded-md ${
                              service.tipo_veiculo === 'MOTO' ? 'bg-orange-50 text-orange-500' : 
                              service.tipo_veiculo === 'CARRO' ? 'bg-blue-50 text-blue-500' : 
                              'bg-slate-50 text-slate-400'
                            }`}>
                                {service.tipo_veiculo || 'AMBOS'}
                            </span>
                          </div>
                        </div>
                      </div>
                      <p className="text-sm font-black text-slate-800">
                        {service.preco_tipo === 'estimado' && <span className="text-[9px] text-slate-300 mr-1 uppercase">A partir de</span>}
                ...
                {selectedServices.length > 0 && (
                   <button 
                    onClick={() => setStep(4)}
                    className="w-full py-5 bg-slate-900 text-white rounded-[2rem] text-[10px] font-black uppercase tracking-[0.2em] shadow-xl mt-6 active:scale-[0.98] transition-all"
                  >
                    Continuar para Resumo
                  </button>
                )}
                <button onClick={() => setStep(2)} className="text-xs font-bold text-primary uppercase tracking-widest py-4 hover:underline w-full text-center">← Voltar para seleção de veículo</button>
              </div>
            )}
            ...
            {step === 4 && (
              <div className="space-y-8 animate-slideInRight">
                        ...
                        {(() => {
                          ...
                          return (
                            <p className="font-bold text-slate-700">
                              {v ? `${v.marca} ${v.modelo} (${v.placa})` : 'Veículo não carregado'}
                            </p>
                          );
                        })()}
                      </div>
                  ...
                  <div>
                    ...
                    <div className="space-y-2">
                      {selectedServices.map(sId => {
                        ...
                        return (
                          <div key={sId} className="flex justify-between items-center text-sm font-semibold text-slate-600 bg-white p-3 rounded-xl border border-slate-100 italic transition-all hover:border-primary/20">
                            <span>{s.nome}</span>
                    ...
                    <div className="flex justify-between items-center">
                      <div className="flex flex-col">
                        ...
                        <div className="flex items-center gap-2">
                          <input 
                            type="number"
                            className="bg-transparent border-b-2 border-slate-100 focus:border-rose-300 outline-none text-sm font-black text-rose-500 w-16 transition-all text-center"
                            value={desconto}
                            min="0"
                            max="100"
                ...
                <div className="space-y-3">
                    <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1">Observações Privadas</label>
                    <textarea 
                        ...
                        placeholder="Notas sobre o orçamento (ex: cliente solicitou desconto, observação sobre a pintura)..."
                        value={observacoes}
                ...
                <div className="flex gap-4">
                  <button 
                    onClick={() => setStep(3)}
                    className="px-8 py-5 text-slate-400 font-black uppercase text-[10px] tracking-widest hover:bg-slate-50 rounded-[2rem] transition-all"
                  >
                    Alterar Serviços
                  </button>
                  <button 
                  ...
                  >
                    {isSaving ? (
                        ...
                        <><Save size={16} strokeWidth={3} /> Salvar e Gerar Proposta</>
                    )}
                  </button>
                </div>
              </div>
